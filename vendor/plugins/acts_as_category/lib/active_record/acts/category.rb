    ...
    module Category
      ...
      module ClassMethods
        ...
        def acts_as_category(params = {})
          ...
          named_scope :permitted, lambda {
            if permissions.empty?
              { :conditions => "#{hidden_column} IS NULL OR #{hidden_column} = 'f'", :order => order_by }
            else
              { :conditions => ["#{hidden_column} IS NULL OR #{hidden_column}= 'f' OR id IN (?)", class_variable_get(:@@permissions)], :order => order_by }
            end
          }
